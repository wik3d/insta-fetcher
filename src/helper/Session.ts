...
export const getCsrfToken = async (): Promise<csrfToken> => {
	...
	try {
		...
		const csrf_Token: csrfToken = headers['set-cookie']?.find(x => x.match(/csrftoken=(.*?);/)?.[1])?.match(/csrftoken=(.*?);/)?.[1] || '';
		return csrf_Token;
	}
	catch (error) {
		throw error;
...
export const getSessionId = async (username: username, password: password): Promise<IgCookie> => {
	...
	try {
		...
		if (authenticated) {
			const Cookie = formatCookie(headers['set-cookie']) || '';
			return Cookie;
		}
		else {
